...
class ApiCall(object):
    ...
    def __init__(self, profile=DEFAULT_PROFILE,
                 ...
                 ephemeral_ak_duration=DEFAULT_EPHEMERAL_AK_DURATION_S,
                 interactive=False):
    ...
    def ephemeral_auth_ak_create(self):
        ...
        ak = None
        sk = None
        expiration_date = (datetime.datetime.now() + datetime.timedelta(seconds=self.ephemeral_ak_duration)).isoformat()
        try:
            call = OSCCall(profile=self.profile_name, login=self.login, password=self.password, authentication_method='password')
            call.make_request('CreateAccessKey', ExpirationDate=expiration_date)
            r = call.response.get("AccessKey")
            if r != None:
                ak = r.get("AccessKeyId")
                sk = r.get("SecretKey")
                res = True
        except Exception  as e:
            print(e)
        return res, ak, sk, expiration_date
...
class OSCCall(JsonApiCall):
    ...
    def build_basic_auth(self):
        if self.authentication_method == "password":
            ...
            basic_auth = "Basic {}".format(base64.urlsafe_b64encode(creds.encode('utf-8')).decode('utf-8'))
            return {
...
def api_connect(service, call, profile=DEFAULT_PROFILE, login=None, password=None,
                authentication_method=DEFAULT_AUTHENTICATION_METHOD,
                ephemeral_ak_duration=DEFAULT_EPHEMERAL_AK_DURATION_S,
                interactive=False,
                *args, **kwargs):
